// Append gotr secret rare index entries (gotr-99 through gotr-132) to data/index.json
// Run: node scripts/index_gotr_secrets.js
'use strict';
const fs   = require('fs');
...
const secrets = gotr.filter(c => parseInt(c.id.replace('gotr-', '')) >= 99);
...
console.log(`Added ${toAdd.length} gotr secret rare entries`);
