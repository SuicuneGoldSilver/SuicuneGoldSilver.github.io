// Append seb secret rare index entries (seb-107 through seb-138) to data/index.json
// Run: node scripts/index_seb_secrets.js
'use strict';
const fs   = require('fs');
...
const secrets = seb.filter(c => parseInt(c.id.replace('seb-', '')) >= 107);
...
console.log(`Added ${toAdd.length} seb secret rare entries`);
